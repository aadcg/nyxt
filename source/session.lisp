  ...
  (handler-case
      (match (with-data-file (input path
        ...
        ((list version history-tree)
         (let* ((buffer-histories (buffer-local-histories-alist history-tree))
                (most-recent-nodes (mapcar (alex:compose
                                            #'first
                                            (alex:rcurry #'sort :test #'local-time:timestamp>
           ...
           (when history-tree
             ...
             (loop for node in most-recent-nodes
                   ...
                   do (dolist (history-node (assoc (id (htree:data node)) buffer-histories))
